class Calendar {
  constructor({
    ...
    this._setHours = hours => this.state.hours = hours;
  }
  ...
  _createMonth = ({
    ...
    const renderedDays = days.map((item, index) => {
      ...
      const disableDay = this._checkDayState({
        day: dateDay,
        month,
        year
      });
      ...
      return `
        <li id="cs-calendar-day-${this.state.id}-${currentDate}" ${firstDayStyle}>
          <a
            ...
            class="${this.state.id}-day-container ${currentDayClass ? `selected` : ``} ${disableDay}"
            data-date="${currentDate}"
            data-disabled="${disableDay}"
    ...
    const calendarDisabled = this.state.disabled ? `disabled` : ``;

    const calendar = `
      <div id="${this.state.id}" class="cs-calendar ${calendarDisabled}">
        ...
        ${this.state.disabled ? `
          <div class="cs-calendar-disabled-content">
            <p>Este proveedor no ha habilitado su calendario</p>
          </div>
        ` : ``}
      </div>
    `;
    ...
    if (!this.state.disabled) document.querySelectorAll(`.${this.state.id}-day-container`).forEach(item => {
      const disabled = item.getAttribute('data-disabled');

      if (disabled !== 'disabled') item.addEventListener('click', () => {
        ...
        const hours = this.state.hours[date];
  ...
  addDateStatus = dateStatus => {
    ...
    dateStatus.map(item => {
      ...
      if (dateContainer) {
        ...
        if (item.hours && !this.state.disabled) {
          //dateContainer.classList.add('cs-tooltip');
          //dateContainer.innerHTML += item.hours;

          hours[date] = item.hours;
        }
      }
    });
  ...
  _checkDayState = ({
    ...
    const currentDate = new Date(this.state.currentYear, this.state.currentMonth, this.state.currentDay);
