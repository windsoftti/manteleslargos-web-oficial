...
$query = "SELECT
  ...
  WHERE C.idCotizacion = $quote_id
";

$query_result       = mysqli_query($mysqli, $query);
$quote_data         = mysqli_fetch_array($query_result);

$today_date         = date('d/m/Y');
$tipo_evento        = $quote_data['TipoEvento'];
$paquete            = $quote_data['Paquete'];
$precio_paquete     = $quote_data['PrecioPaquete'];
$modalidad_paquete  = $quote_data['ModalidadPaquete'];
$fecha_cotizacion   = $quote_data['FechaCotizacion'];
$folio              = $quote_data['Folio'] ? $quote_data['Folio'] : 'M-0000/2022';

$cliente            = $quote_data['NombreCompleto'];
$cliente_telefono   = $quote_data['Telefono'];
$cliente_correo     = $quote_data['Email'];
...
class MYPDF extends TCPDF
{
  ...
  public function Header()
  {
    ...
    $proveedor            = strtoupper($quote_data['Proveedor']);
    $direccion_negocio    = strtoupper($quote_data['DireccionCompleta']);
    $telefono_proveedor   = $quote_data['TelefonoProveedor'];
    $whatsapp_proveedor   = $quote_data['CelularProveedor'];
    $facebook_proveedor   = $quote_data['Facebook'];
    $instagram_proveedor  = $quote_data['Instagram'];
    // Logo
