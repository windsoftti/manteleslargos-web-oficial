...
const cscCreateCalendar = ({
  ...
  return new Promise((resolve, reject) => {
    ...
    const renderedMonths = calendar.map(({
      ...
      return `
        <a id="cs-inline-month-${monthKey}" class="cs-inline-month ${activeMonth}" href="javascript:void(0)" onclick="cscChangeMonth(${monthKey})">
          <h3>${currentYear}</h3>
          <h1>${monthName}</h1>
        </a>
      `;
      ...
      const renderedDays = days.map((day, index) => {
        ...
        return `
          ...
          >
            <div class="actions">
              <a
                class="plus csc-add-event"
                data-date="${currentDate}"
                href="javascript:void(0)"
              ></a>

              <a
                class="unlock csc-unlock-event"
                data-date="${currentDate}"
                href="javascript:void(0)"
              ></a>
            ...
            <div id="csc-${currentDate}" class="events"></div>
          </li>
        `;
      }).join('');
    ...
    document.querySelectorAll('.csc-add-event').forEach(btn => {
      btn.addEventListener('click', function () {
        const date = this.getAttribute('data-date');
        !!handleAdd ? handleAdd(date) : alert(date);
      });
    });

    /* document.querySelectorAll('.csc-add-reminder').forEach(btn => {
      btn.addEventListener('click', function () {
        const date = this.getAttribute('data-date');
        !!handleAddReminder ? handleAddReminder(date) : alert('Para utilizar los recordatorios, debes de actualizar al plan Básico.');
      });
    }); */

    document.querySelectorAll('.csc-unlock-event').forEach(btn => {
      btn.addEventListener('click', function () {
        const date = this.getAttribute('data-date');
        !!handleUnlock ? handleUnlock(date) : alert(date);
      });
    });
...
const cscReminderItem = ({
  ...
  <a class="btn-csc-reminder" href="javascript:void(0)" data-info="${data}" data-date="${date}" title="${description}" style="background-color: ${color}">
    <span>
      ${title}
    </span>
  </a>
...
const cscAddReminders = reminders => reminders.map(reminder => {
  const dates = reminder.dates;
  const title = reminder.title;
  const description = reminder.description;
  const color = reminder.color;
  const data = btoa(JSON.stringify(reminder));

  dates.map(date => {
    const dateId = `csc-${date}`;
    const reminderContainer = document.getElementById(dateId);

    const reminderItem = cscReminderItem({
      title,
      description,
      data,
      date,
    ...
    reminderContainer.innerHTML += reminderItem;
  });
});
