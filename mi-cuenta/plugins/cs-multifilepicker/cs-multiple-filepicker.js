class CSMultipleFilepicker {
  ...
  createImagePreview = (image, onRemove) => {
    ...
    const removeFromInitialImages = index => this._removeFromInitialImages(index);
    ...
    if (!onRemove) {
      ...
      buttonRemove.addEventListener('click', function (e) {
        ...
        if (imageId) initialImages.map((item, index) => {
          if (imageId == item) removeFromInitialImages(index)
        });
  ...
  initListeners = () => {
    ...
    const getInitialImagesLength = () => this._getInitialImagesLength();
    const getCSFilesLength = () => this._getCSFilesLength();
    ...
    const clickButton = (e) => {
      ...
      const initialImagesLength = getInitialImagesLength();
      const csFilesLength = getCSFilesLength();
    ...
    inputTag.addEventListener('change', function (event) {
      ...
      document.getElementById(`cs-multiple-filepicker-input-${id}`).value = "";
    });
  }
  /* if(imageId) initialImages.map((item, index) => {
  ...
  _setInitialImages = imageId => this.state.initialImages.push(imageId);
  _getInitialImagesLength = () => this.state.initialImages.length;
  _removeFromInitialImages = index => this.state.initialImages.splice(index, 1);

  _getCSFilesLength = () => this.state.csFiles.length;
  ...
  addImages = ({ images = [], onRemove }) => {
    ...
    images.map(image => {
      ...
      this._setInitialImages(image.imageId);
    });
  }
