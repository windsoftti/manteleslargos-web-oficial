...
$file_url     = $images_absolute_url . 'eventosRecientes/galeria/';
$file_folder  = $images_url . 'eventosRecientes/galeria/';

switch ($action) {
  case 'list_gallery':
    $recent_events_id = cleanStr($_POST['recentEventId']);

    $query = "SELECT idGaleria, idEvento, Imagen FROM galeria_eventos_recientes WHERE idEvento = '$recent_events_id'";
...
<?php
  ...
  case 'delete_image_galery':
    ...
    if ($delete_file === 'deleted' || $delete_file === 'not-exist') {
      $query        = "DELETE FROM galeria_eventos_recientes WHERE idGaleria = '$file_id' AND Imagen = '$file'";
      $query_result = mysqli_query($mysqli, $query);
